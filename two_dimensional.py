...
# a homogenous version of the model 
# for each node having two opinions and each node having the same fixed c, h and a
...
def initialize():
    ...
    for i in g.nodes:
        g.nodes[i]['state'] = [normal(0,1)] * D # each node's opinion
...
def update():
    ...
    for i in nodes:
        ...
        if total > 0:
            # conformity
            av = [sum(g[i][j]['weight'] * g.nodes[j]['state'][0] for j in nbs) / total , sum(g[i][j]['weight'] * g.nodes[j]['state'][1] for j in nbs) / total]
            g.nodes[i]['state'][0] += g.nodes[i]['c'] * (av[0] - g.nodes[i]['state'][0]) * Dt
            g.nodes[i]['state'][1] += g.nodes[i]['c'] * (av[1] - g.nodes[i]['state'][1]) * Dt

            # neophily
            for j in nbs:
                max_diff = max([abs(av[d] - g.nodes[j]['state'][d]) for d in range(D)])
                g[i][j]['weight'] += g.nodes[i]['a'] * (max_diff - g.nodes[i]['theta_a']) * Dt

        # homophily
        for j in nbs:
            diff = max([abs(g.nodes[i]['state'][d] - g.nodes[i]['state'][d]) for d in range(D)])
        ...
        # random drift
        g.nodes[i]['state'] += [normal(0, epsilon) , normal(0, epsilon)]
