...
def build(c, h, a, theta_h, theta_a):
    for d in D:
        for ii in range(II):
            filename = f"output/{d}-{c}-{h}-{a}-{theta_h}-{theta_a}-{ii}.pkl"
            ...
            if os.path.exists(filename): 
                ...
                initialize(c, h, a, theta_h, theta_a, "max", "min", d)
                t = T / 100
                ...
                avg_edgw, com, mod_com, range_avg_com, sd_avg_com = get_statistics(g, d)
                store_statistics(d, ii, c, h, a, theta_h, theta_a, avg_edgw, com, mod_com, range_avg_com, sd_avg_com)
                pickle.dump(g, open(filename, 'wb'))


def store_statistics(dim, ii, c, h, a, theta_h, theta_a, avg_edgw, com, mod_com, range_avg_com, sd_avg_com):
    with open('max_min.csv', 'a') as stats:
        writer_object = writer(stats)
        writer_object.writerow([dim, ii, c, h, a, theta_h, theta_a, avg_edgw, com, mod_com, range_avg_com, sd_avg_com])
    stats.close()


# get statistics of a network
def get_statistics(dir_graph, d):
    ...
    if avg_edgw == 0: # no node is connected to any other node
        num_comms = n # number of comms is then number of nodes (each node is its own comm)
        mod = 1 
        range_avgcomms = [] # range in each dimension 
        std_avgcomms = [] # std in each dimension 
        for dim in range(d):
            opinions = [g.nodes[i]['state'][dim] for i in g.nodes] # gets state of each node in dim d 
            range_avgcomms.append(max(opinions) - min(opinions)) # finds range in dim d 

            opinions_mean = sum(opinions) / len(opinions) # gets average state in dim d 
            opinions_var = (1/len(opinions)) * sum([((opinion - opinions_mean) ** 2) for opinion in opinions]) # gets variance in dim d of states
            std_avgcomms.append(opinions_var ** 0.5) # std of states in dim d

        range_avgcomm = sum(range_avgcomms) / len(range_avgcomms)
        std_avgcomm = sum(std_avgcomms) / len(std_avgcomms)

        return avg_edgw, num_comms, mod, range_avgcomm, std_avgcomm
...
def initialize(c, h, a, theta_h, theta_a, h_update, a_update, d): # add h_strat and a_strat
    global g
    ...
    for i in g.nodes: # first point of change made here ! 
        ...
        for dim in range(d):
            ...
            g.nodes[i]['state'][dim] += normal(0, 1) # each node's opinion
...
def update(d):
    ...
    for i in nodes:
        ...
        if total > 0:
            ...
            for dim in range(d):
                avs.append(sum(g[i][j]['weight'] * g.nodes[j]['state'][dim] for j in nbs) / total)
            

            # conformity
            # iterate over the D dimensions applying conformity to each one
            for dim in range(d):
                g.nodes[i]['state'][dim] += g.nodes[i]['c'] * (avs[dim] - g.nodes[i]['state'][dim]) * Dt

            # neophily
            for j in nbs:
                diffs = [] # differences of node j from local average in each dimension
                for dim in range(d):
                    diffs.append(abs(avs[dim] - g.nodes[j]['state'][dim])) # calculated how far j is from local average in each dimension
        ...
        # homophily
        for j in nbs:
            diffs = [abs(g.nodes[i]['state'][dim] - g.nodes[j]['state'][dim]) for dim in range(d)] # stores difference between node i and j in each dimension
        ...
        for dim in range(d):
            g.nodes[i]['state'][dim] += normal(0, epsilon)
